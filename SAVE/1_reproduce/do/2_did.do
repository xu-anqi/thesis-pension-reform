/***********************************************************************
                        2_did.do
***********************************************************************/

*──────── housekeeping ───────────────────────────────────────────────*
clear all
set more off
capture log close

*── paths ─────────────────────────────────────────────────────────────
global DIR       "~/Desktop/thesis/data/SAVE/1_reproduce"
global DATA      "$DIR/data"
global OUT       "$DIR/output"
global LOG       "$DIR/log"

log using "${LOG}/2_did.log", replace

*──────── 0. load cleaned panel ──────────────────────────────────────*
use "${DATA}/data.dta", clear        // produced by 1_append.do


*──────── 1. OLS ──────────────────────────────────────*
* basic
reg era treated post06 interaction06, robust


* add demo edu employ
reg era treated post06 interaction06 ///
    female age partner female_partner east_germany /// demo
    i.education currently_unemployed i.past_employment /// edu + employment
    , robust

	
* add income, wealth
reg era treated post06 interaction06 ///
    female age partner female_partner east_germany /// demo
    i.education currently_unemployed i.past_employment /// edu + employment
	financial_wealth financial_wealth_sq private_old_age_provision ///
	real_estate household_income household_income_sq ///income & wealth
    , cluster(respid)




* add heath, expectation	
reg era treated post06 interaction06 ///
    female age partner female_partner east_germany /// demo
    i.education currently_unemployed i.past_employment /// edu + employment
	financial_wealth financial_wealth_sq private_old_age_provision ///
	real_estate household_income household_income_sq ///income & wealth
	health_notgood expected_inheritance expected_health_worsening ///
	expected_income_increase expected_unemployment life_expectancy ///
	exp_replacement_rate na_expected_replacement unsatisfied_current_job ///health
    , robust



* add panel attrition
xtset respid year
xtreg era treated post06 interaction06 ///
    female age partner female_partner east_germany /// demo
    i.education vocational_training /// edu
	currently_unemployed i.past_employment /// employment
	financial_wealth financial_wealth_sq private_old_age_provision ///
	real_estate household_income household_income_sq ///income & wealth
	health_notgood expected_inheritance expected_health_worsening ///
	expected_income_increase expected_unemployment life_expectancy ///
	exp_replacement_rate na_expected_replacement unsatisfied_current_job ///health
	s05 s06 s07 s08 /// panel attrition
    , cluster(respid)
	
xtset respid year
xtreg era treated post06 interaction06 ///
    female age partner female_partner east_germany /// demo
    i.education vocational_training /// edu
	currently_unemployed i.past_employment /// employment
	financial_wealth financial_wealth_sq private_old_age_provision ///
	real_estate household_income household_income_sq ///income & wealth
	health_notgood expected_inheritance expected_health_worsening ///
	expected_income_increase expected_unemployment life_expectancy ///
	unsatisfied_current_job ///health
	s05 s06 s07 s08 /// panel attrition
    , cluster(respid)
	

estimates store ols

*──────── 2. Heckman ──────────────────────────────────────*
gen has_era = !missing(era)   // TRUE if not . or .a etc.


xtset respid year
heckman era treated post06 interaction06 ///
    female age partner female_partner east_germany /// demo
    i.education vocational_training /// edu
	currently_unemployed i.past_employment /// edu + employment
	financial_wealth financial_wealth_sq private_old_age_provision ///
	real_estate household_income household_income_sq ///income & wealth
	health_notgood expected_inheritance expected_health_worsening ///
	expected_income_increase expected_unemployment life_expectancy ///
	exp_replacement_rate na_expected_replacement unsatisfied_current_job ///health 
	s05 s06 s07 s08 /// panel attrition
    , select (na_exp_econ na_exp_health na_exp_increase_income /// 
	na_exp_unemploy na_exp_life_expectancy_1 na_exp_life_expectancy_2 ///
	na_exp_life_expectancy_3 /// exclusion
	female age i.education /// demo
	) cluster(respid)
	
xtset respid year
heckman era treated post06 interaction06 ///
    female age partner female_partner east_germany /// demo
    i.education vocational_training /// edu
	currently_unemployed i.past_employment /// edu + employment
	financial_wealth financial_wealth_sq private_old_age_provision ///
	real_estate household_income household_income_sq ///income & wealth
	health_notgood expected_inheritance expected_health_worsening ///
	expected_income_increase expected_unemployment life_expectancy ///
	unsatisfied_current_job ///health
	s05 s06 s07 s08 /// panel attrition
    , select (na_exp_econ na_exp_health na_exp_increase_income /// 
	na_exp_unemploy na_exp_life_expectancy_1 na_exp_life_expectancy_2 ///
	na_exp_life_expectancy_3 /// exclusion
	female age i.education /// demo
	) cluster(respid)
	
estimates store heckman1

xtset respid year
heckman era treated post06 interaction06 ///
    female age partner female_partner east_germany /// demo
    i.education vocational_training /// edu
	currently_unemployed i.past_employment /// edu + employment
	financial_wealth financial_wealth_sq private_old_age_provision ///
	real_estate household_income household_income_sq ///income & wealth
	health_notgood expected_inheritance expected_health_worsening ///
	expected_income_increase expected_unemployment life_expectancy ///
	exp_replacement_rate na_expected_replacement unsatisfied_current_job ///health 
	s05 s06 s07 s08 /// panel attrition
	i. year /// year fe
    , select (na_exp_econ na_exp_health na_exp_increase_income /// 
	na_exp_unemploy na_exp_life_expectancy_1 na_exp_life_expectancy_2 ///
	na_exp_life_expectancy_3 /// exclusion
	female age i.education /// demo
	) cluster(respid)
	
xtset respid year
heckman era treated interaction06 ///
    female age partner female_partner east_germany /// demo
    i.education vocational_training /// edu
	currently_unemployed i.past_employment /// edu + employment
	financial_wealth financial_wealth_sq private_old_age_provision ///
	real_estate household_income household_income_sq ///income & wealth
	health_notgood expected_inheritance expected_health_worsening ///
	expected_income_increase expected_unemployment life_expectancy ///
	unsatisfied_current_job ///health
	s05 s06 s07 s08 /// panel attrition
	i. year /// year fe
    , select (na_exp_econ na_exp_health na_exp_increase_income /// 
	na_exp_unemploy na_exp_life_expectancy_1 na_exp_life_expectancy_2 ///
	na_exp_life_expectancy_3 /// exclusion
	female age i.education /// demo
	) cluster(respid)
	
estimates store heckman2

*--------------------- 07 cutoff
xtset respid year
heckman era treated post07 interaction07 ///
    female age partner female_partner east_germany /// demo
    i.education vocational_training /// edu
	currently_unemployed i.past_employment /// edu + employment
	financial_wealth financial_wealth_sq private_old_age_provision ///
	real_estate household_income household_income_sq ///income & wealth
	health_notgood expected_inheritance expected_health_worsening ///
	expected_income_increase expected_unemployment life_expectancy ///
	unsatisfied_current_job ///health
	s05 s06 s07 s08 /// panel attrition
    , select (na_exp_econ na_exp_health na_exp_increase_income /// 
	na_exp_unemploy na_exp_life_expectancy_1 na_exp_life_expectancy_2 ///
	na_exp_life_expectancy_3 /// exclusion
	female age i.education /// demo
	) cluster(respid)
	
estimates store heckman0


*Display Results Side-by-Side
esttab heckman0 heckman1 heckman2 ols using "${OUT}/DID.txt", ///
    replace se b(3) star(* 0.10 ** 0.05 *** 0.01) ///
    title("Results") ///
    label stats(chi2 p N r2_w, fmt(0 3 0 3) ///
          labels("Wald $\chi^2$" "p-value" "Observations" "R-squared (within)"))






log close
