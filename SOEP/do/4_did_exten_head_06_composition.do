...
global OUT_OUTPUT "$AVZ/output/4_exten_head/"
...
esttab logit1 logit2 logit3 logit4 logit5 logit6 using "${OUT_OUTPUT}did_if_savings_logit_06.txt", ///
    replace se b(3) star(* 0.10 ** 0.05 *** 0.01) ///
    title("Extensive Margin: Effect of the 2006 Pension Reform on Savings (Logit) 06 cut-off") ///
    label

esttab logit1 logit2 logit3 logit4 logit5 logit6 using "${OUT_OUTPUT}did_if_savings_logit_06.tex", ///
    replace se b(3) star(* 0.10 ** 0.05 *** 0.01) ///
    title("Extensive Margin: Effect of the 2006 Pension Reform on Savings (Logit) 06 cut-off") ///
...
* Display Results Side-by-Side
esttab probit1 probit2 probit3 probit4 using "${OUT_OUTPUT}did_if_savings_probit_06_composition.txt", ///
    replace se b(3) star(* 0.10 ** 0.05 *** 0.01) ///
...
esttab ame1 ame2 ame3 ame4 using "${OUT_OUTPUT}ame_probit_composition.txt", ///
