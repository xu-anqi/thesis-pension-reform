...
global MY_OUTPUT_OUT "$AVZ/output/4_inten_head/"

*log using "${OUT_LOG}4_did_inten_head_06.log", replace
...
reg log interaction post type_employment, vce(cluster hid)
...
did_multiplegt (dyn) log hid syear interaction, cluster(hid) placebo(1)
graph export "${MY_OUTPUT_OUT}did_multiplegt_06.png", replace
...
esttab lpm1 lpm2 lpm3 lpm4 lpm5 lpm6 using "${MY_OUTPUT_OUT}did_amount_06_composition.txt", ///
...
esttab lpm1 lpm2 lpm3 lpm4 lpm5 lpm6 using "${MY_OUTPUT_OUT}did_amount_06_composition.tex", ///
    replace se b(3) star(* 0.10 ** 0.05 *** 0.01) ///
    title("Intensive Margin: Effect of the 2006 Pension Reform on Amount Saved 06") ///
...
reg rate interaction post type_employment, vce(cluster hid)
estimates store lpm1
...
* Display Results Side-by-Side
esttab lpm1 lpm2 lpm3 lpm4 lpm5 lpm6 using "${MY_OUTPUT_OUT}did_rate_06_composition.txt", ///
    replace se b(3) star(* 0.10 ** 0.05 *** 0.01) ///
    title("Intensive Margin: Effect of the 2006 Pension Reform on Saving Rate 06") ///
    label stats(r2 N, fmt(3 0) labels("R-squared" "N"))

esttab lpm1 lpm2 lpm3 lpm4 lpm5 lpm6 using "${MY_OUTPUT_OUT}did_rate_06_composition.tex", ///
    replace se b(3) star(* 0.10 ** 0.05 *** 0.01) ///
    title("Intensive Margin: Effect of the 2006 Pension Reform on Saving Rate 06") ///
    label booktabs drop(*.syear)
