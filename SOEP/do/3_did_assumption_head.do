...
global OUT_OUTPUT "$AVZ/output/3_did_assumption_head/" //changed directory to folder

log using "${OUT_LOG}3_did_assumption_head.log", replace
...
collapse (mean) if_savings (sd) if_savings_sd=if_savings (count) N = pid, by(syear type_employment)
...
collapse (mean) savings_amount (sd) savings_sd=savings_amount (count) N = pid, by(syear type_employment)
...
collapse (mean) log_savings (sd) log_savings_sd=log_savings (count) N = pid, by(syear type_employment)
...
collapse (mean) saving_rate (sd) saving_rate_sd = saving_rate (count) N = pid, by(syear type_employment)
